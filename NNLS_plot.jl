...
DataFile1 = @sprintf "%s/g_%.1f_D_%i_beta_%i.txt" DataFolder g D β
DataFile2 = @sprintf "%s/g_%.1f_D_%im2_beta_%i.txt" DataFolder g D β
...
plot(rn2, sn2, line=(2),label="D=8"
    ,xaxis=:log, xlabel="Residual(K*S-y) norm"
    ,yaxis=:log, ylabel="Solution(S) norm"
    ,framestyle=:box)
add_sigle_point(60, d2)
add_sigle_point(100, d2)
add_sigle_point(150, d2)
FigureName = @sprintf "ising_Lcurve_g_%.1f_D_%im2_beta_%i.pdf" g D β
...
function add_line(num::Int64, data::AbstractArray)
    lambda_value = @sprintf "λ=%.5e" data[num, 1]
    path = @sprintf "%s/%s/g_%.1f_D_%im2_beta_%i_lambda_%.5e.txt" NNLSFolder func g D β data[num, 1]
    d = readdlm(path)
...
add_line(100, d2)
add_line(140, d2)
FigureName = @sprintf "ising_Sw_g_%.1f_D_%im2_beta_%i.pdf" g D β
savefig(FigureFolder*"/"*FigureName)
