...
DataFile2 = @sprintf "%s/g_%.1f_D_%i_beta_%i_dG.txt" DataFolder g D β
...
add_sigle_point(190, d1)
FigureName = @sprintf "ising_Lcurve_g_%.1f_D_%i_beta_%i_dG.pdf" g D β
...
function add_line(num::Int64, data::AbstractArray)
    lambda_value = @sprintf "λ=%.5e" data[num, 1]
    path = @sprintf "%s/%s/g_%.1f_D_%i_beta_%i_lambda_%.5e_dG.txt" NNLSFolder func g D β data[num, 1]
    d = readdlm(path)
...
add_line(130,d1)
add_line(150, d1)
FigureName = @sprintf "ising_Sw_g_%.1f_D_%i_beta_%i_dG.pdf" g D β
savefig(FigureFolder*"/"*FigureName)
