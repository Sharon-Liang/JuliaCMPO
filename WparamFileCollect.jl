...
for b in 1:length(betaRange)
    ...
    ResultFile1 = @sprintf "%s/g_%.1f_D_%i_beta_%i_Akernel.txt" DataFolder g D β 
    ResultFile2 = @sprintf "%s/g_%.1f_D_%im2_beta_%i_Akernel.txt" DataFolder g D β
    
    for i in 1:length(lambdaRange)
        λ = lambdaRange[i]
        DataFile1 = @sprintf "%s/g_%.1f_D_%i_beta_%i_lambda_%.5e_Akernel.txt" DataFolder g D β λ
        DataFile2 = @sprintf "%s/g_%.1f_D_%im2_beta_%i_lambda_%.5e_Akernel.txt" DataFolder g D β λ
